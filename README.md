...
# Note for Free cloud
当アプリは php-mbstring（マルチバイト文字処理の関数群）を利用しています。通常のPHPが利用できるレンタルサーバーであれば、ほとんどの場合あらかじめインストールされていますので意識する必要はありませんが、クラウドサーバーで利用する場合はインストールが必要です。Herokuでは下記のように **composer.json** に require で追加してください。<br>
