...
また、クラウドサービスの制限によりメール送信用の mb_send_mail関数が利用できない場合は **SendGrid**を利用します。（事前にSendGridのアカウントとAPIキーを取得し、利用するクラウドサービス上にて環境変数の設定がしてあるのが前提です。）これも下記のようにcomposer.json に require で追加してください。<br>
